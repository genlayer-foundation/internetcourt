...
- SHIPPED TO PRODUCTION 2026-07-21. Website: commit `befbc7c` on `feat/marketing-one-pager`, pushed to `genlayer-foundation/internetcourt`. Skill: commit `1b32438` on `main`, pushed to `internet-court/buro` (SSH keys were not loaded in the agent, so that push went over HTTPS using the `gh` credential helper; only the 3 humanode paths were staged, other sessions' work left untouched). Deployed staging first (`internetcourt-df2p9pf1f`, aliased to staging.internetcourt.org), then prod on the user's explicit "deploy to prod" (`internetcourt-4hxog7k6a`, aliased to internetcourt.org).
- Verified on prod: homepage HTML references `partners/humanode.svg`, the asset returns 200 `image/svg+xml` and its sha256 is byte-identical to the committed source, old `humanode.png` returns 404. Locally screenshotted in the founding-members grid at 151x26px (gridHeight 15 x SCALE 1.75), reading at the same optical weight as OpenServ and Humanity Protocol beside it.
...
- 3 edits per locale file: (1) roster lead-in en:108 "founding ecosystem"/"partners came together" → "ecosystem"/"Internet Court brings together solutions by companies across every layer of the stack"; (2) en:117 "All 28 founding members" → "All 28 participating companies and solutions... working as one..."; (3) en:119 image alt "founding members" → "companies and solutions in the Internet Court ecosystem".
- Plan: `/Users/rasca/.claude/plans/we-need-to-modify-peaceful-waterfall.md`. Status: DONE (uncommitted, 2026-07-13). All 5 locales edited (3 lines each), grep-verified: no "founding member(s)"/"founding ecosystem" phrasing left in the article in any locale (es "fundador", ko "창립", zh "创始", ru "учредител/основател" all zero hits), MetaMask mentions untouched, no em dashes introduced. Note: es/ko/zh/ru copies are pipeline-generated; en source changed so next build re-translates those passages anyway (hand-edits keep live content correct meanwhile). NOT committed, NOT deployed; working tree holds much unrelated uncommitted work from other sessions, so commit/deploy needs care.

### www redirect + Vercel Analytics (2026-07-10, on prod)
- www.internetcourt.org didn't resolve (never added; DNS at Cloudflare, NS cash/elisabeth.ns.cloudflare.com). Added www to Vercel project `internetcourt` via CLI + host-based 308 redirect www->apex in `frontend/next.config.ts` (`redirects()`, matches only www host, path-preserving). Committed `799353a`, deployed prod. Edge-tested via Host header: 308 -> https://internetcourt.org/faq; apex still 200. REMAINING (user, Cloudflare): add `A www.internetcourt.org 76.76.21.21` (or CNAME www -> cname.vercel-dns.com), DNS-only.
- Vercel Analytics: `npm i @vercel/analytics` (^2.0.1) + `<Analytics/>` from `@vercel/analytics/next` in root `[locale]/layout.tsx` (loads all pages/locales, alongside gtag). Committed `b1def6b`, deployed prod. Verified `/_vercel/insights/script.js` -> 200 (Web Analytics enabled on project). Note: injected client-side so not in raw SSR HTML (expected).

### Dark-mode load flash (FOUC) — FIXED + on prod
- Symptom: "everything flashes like a big SVG / unstyled page for a frame" on refresh and after deploy. Root cause: site had no `color-scheme` declared, so on OS/browser DARK MODE the browser painted its dark default canvas until the site CSS loaded, then replaced it with the paper page = full-page dark flash. Never reproduced in default light-mode headless; confirmed via CPU-throttled dark-mode Playwright capture (unpatched: ~13 black frames before paint; patched: flips to light at document-parse).
- Fix (committed `b215ccb`, pushed to main → deployed prod 2026-07-10): `color-scheme: light` + paper `#f7f4ec` background on `<html>`, applied BOTH inline in `[locale]/layout.tsx` (`<html style=...>`, so it holds before external CSS parses) and in globals.css `@layer base html {}`. Prod serves the inline style (verified via curl). tsc clean.

### Google Analytics — environment-specific gtag.js
- DONE + DEPLOYED TO STAGING (committed `07eeae6`, pushed, tsc clean, 2026-07-09): gtag.js added to root layout `src/app/[locale]/layout.tsx` via `next/script` (afterInteractive), fires on every page across all locales. Measurement ID selected by `process.env.VERCEL_ENV`: production=`G-89MRCLMMC5`, preview/staging=`G-XGHQ9399SG` (was G-JB6HWBHGF8), local dev=none. No new deps, no consent banner (matches privacy policy copy). Deployed `internetcourt-8grree83r` aliased to staging.internetcourt.org (preview env → staging GA ID). Staging is basic-auth gated (BASIC_AUTH_PASSWORD) so curl verification returns 401; verify GA + /privacy in-browser with the staging password.
...
- Round 7 (DONE, uncommitted, tsc clean, 2026-07-09): all generic/AI-ish icons replaced with official Remix Icons per Pablo. New shared `src/components/icons/remix.tsx` (16 icons, official 24x24 path data embedded, currentColor, no webfont, no new deps). Swapped: blog blockquote quote char → ri-double-quotes-l (the screenshot Pablo flagged); header socials → ri-telegram/github/twitter-x-line; lang toggle, SkillCommand (copy/check/external/send), video play (ri-play-fill) + mute, scroll-cue arrows, "All posts" arrow, brand page TOC/do-dont checks. Brand marks + partner logos untouched. `lucide-react` now fully unused (still in package.json, removable). FLAG: brand-book copy in messages/en.json (~line 441) still says UI glyphs use "lucide" — needs a copy pass (translated key, left alone).
- GIT STATE 2026-07-09: rounds 5+6 committed as `05f977b` on `feat/marketing-one-pager`; `redesign/tic-brand` fast-forwarded to same commit; `main` untouched per Pablo ("do not merge into main"). Branch PUSHED to `genlayer-foundation/internetcourt` (a first attempt 403'd for `pdepablocom` but a retry ~15min later succeeded, so access now works). Also pushed to `internet-court/webpage` as `feat/marketing-one-pager` during the 403 window (remote `webpage`, unrelated history, its `main` = old minimal site, untouched) — that copy is now redundant, delete the branch there if desired.
- Round 6 (DONE, committed in 05f977b, tsc clean, 2026-07-09): hero md-to-lg breakpoint restructured per Pablo (tablet screenshot: stacked card pushed video below fold). md-lg now = row 1: eyebrow+H1 (40px, flex-1) LEFT + SkillCommand card (`md:w-[400px] md:shrink-0`, bottom-aligned `md:items-end`) RIGHT; row 2: video FULL WIDTH directly below (aspect-video, object-contain, gap-8 reused). lg+ pixel-identical to before via reset classes (`lg:flex-col lg:items-stretch lg:gap-0`, `lg:text-[56px]` — the 56px token moved md:→lg:). Below md unchanged stack. One mounted <video>, same DOM order, CSS-only. Verified with headless-chrome screenshots at 500/768/900/1023/1440.
- Round 5 (DONE, committed in 05f977b, tsc clean, 2026-07-09): per Pablo — (1) hero video NEVER cropped: always `aspect-video` + `object-contain` (stretch-to-column/object-cover removed), `self-center` in right column; (2) "BACKED BY" mono label removed from BackedByLogos (label prop dropped; i18n key `home.hero.backedBy` kept in messages, just unrendered); (3) marquee boxes shrunk 218x104 → 152x72 (px-4), 7 visible per 1152px row instead of 5, PartnerLogo sized via `calc(var(--logo-h)*0.72)` overrides (text factor 0.65); (4) video column widened `lg:w-[560px]` → `lg:w-[640px]` so the video outweighs the SkillCommand card (~41/59 split, H1 wraps fine at ~448px).
