...
Different parts of this path are now being covered by different standards, each designed for a specific need, with placeholders where the other layers are meant to plug in. x402 is advancing internet-native payments, but defines no formal dispute mechanism. ERC-8004, the Trustless Agents standard, is advancing agent discovery, identity, and reputation, and leaves adjudication to separate validation protocols. A2A is advancing agent interoperability, but not dispute resolution. Around these standards, partner layers are forming across wallets, smart accounts, contract frameworks, analytics, execution, verification, escrow, and adjudication.
...
The founding ecosystem spans payment rails, wallets, identity and reputation, execution, verification, analytics, blockchain infrastructure, and dispute systems: MetaMask, x402 Foundation, OKX, 0G Labs, zkSync, Starknet, BNB Chain, Nansen, Humanity Protocol, AltLayer, ChainGPT, Anoma, AppLayer, OpenServ, UMA, Chutes, io.net, MEMO, Chainbase, LI.FI, AntSeed, and Arkhai. Each covers a different layer, and they share one conviction: no layer reaches a real economy on its own. That is why they have come together, around a single vision of every protocol and solution working as one toward the same goal, end-to-end agentic commerce.
